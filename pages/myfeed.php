...
$result = mysql_query("SELECT * FROM favorites WHERE user_id='".mysql_real_escape_string($_SESSION['userid'])."' ORDER BY ID DESC LIMIT ".$lim_fav."");
...
$content .= '</div>';
    ...
    $page_template = str_replace("[TITLE]", $my_feed_page_title, $page_template);
    $page_template = str_replace("[H-TITLE]", $my_feed_page_h, $page_template);
